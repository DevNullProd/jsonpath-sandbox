...
Handlers.prototype._fns = {
  ...
  'subscript-child-script_expression': function(component, partial) {
    return new Promise(async function(resolve){
      ...
      resolve(await this._eval_recurse(partial, exp, '$[{{value}}]'));
    }.bind(this))
  },
...
Handlers.prototype._eval_recurse = async function(partial, src, template) {
  var jp = require('./index');
  var expression = this._replace_at(src)
  var value = (await this._evaluate(expression, { _AT_SYMBOL_: partial.value })).value;
...
Handlers.prototype._evaluate = function(expression, globals) {
  ...
  if(!this.sandbox){
    ...
    this.sandbox.execute = _memoize(this.sandbox.execute, JSON.stringify)
  }
  ...
  if(this.expression_timeout)
    execute.timeout = this.expression_timeout

  if(this.globals)
    Object.assign(execute.globals, this.globals)
